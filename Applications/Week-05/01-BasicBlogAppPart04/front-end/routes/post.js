...
router.get('/delete/:id', async function(req, res, next) {
    try {
        ...
        post = new Post(post);
        if (post && post.id) {
            res.render('delete', { csrfToken: req.csrfToken(), id: id, title: post.title, post: post.post, username: post.username, postdate: post.created_at, editdate: post.updated_at });
        } else {
...
router.get('/:id', async function(req, res, next) {
    try {
        ...
        post = new Post(post);
        if (post && post.id) {
            res.render('edit', { csrfToken: req.csrfToken(), id: id, mode: 'Edit Post', submitButton: 'Save Changes', title: post.title, post: post.post, username: post.username, postdate: post.created_at, editdate: post.updated_at });
        } else {
...
async function handlePost(req, res, next) {
    try {
        if (!isNaN(req.body.id) && req.body.id > 0) {
            ...
            post = new Post(post);
            res.render('edit', { csrfToken: req.csrfToken(), id: req.body.id, mode: 'Edit Post', submitButton: 'Save Changes', title: post.title, post: post.post, username: post.username, postdate: post.created_at, editdate: post.updated_at });
        ...
        res.render('edit', { csrfToken: req.csrfToken(), id: req.body.id, mode: 'Edit Post', submitButton: 'Save Changes', title: req.body.title, post: req.body.post, error: e.message });
    }
}

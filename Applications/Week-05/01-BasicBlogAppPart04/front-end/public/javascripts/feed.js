(function() {
    ...
    MyBlogApp.fetchFeed = function() {
        ...
        const by = document.getElementById('orderBy').value;
    ...
    MyBlogApp.postFeed = function(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (postData.created_at !== postData.updated_at) {
                post.querySelector(".post-updated").classList.remove('hidden');
            }
    ...
    window.onload = function() {
        MyBlogApp.fetchFeed();
        document.getElementById('btnRefresh').addEventListener('click', MyBlogApp.fetchFeed);
